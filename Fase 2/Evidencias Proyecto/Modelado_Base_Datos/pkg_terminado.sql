...
CREATE SEQUENCE SEQ_REGISTRO_LLAMADA
    START WITH 1
    INCREMENT BY 1
    NOCACHE
    NOCYCLE;
...
CREATE OR REPLACE TRIGGER TRG_REGISTRO_LLAMADA_ID
BEFORE INSERT ON REGISTRO_LLAMADA
FOR EACH ROW
BEGIN
    IF :NEW.ID_REGISTRO_LLAMADA IS NULL THEN
        SELECT SEQ_REGISTRO_LLAMADA.NEXTVAL
        INTO :NEW.ID_REGISTRO_LLAMADA
        FROM DUAL;
    END IF;
END;
/
